...
- $\mathcal{S}, \mathcal{A}$: state space and action space.
    - $\mathcal{V}$ 是词汇表.
    - 在 LLM 中, $x$ 为一整条问题, $x \in \mathcal{S} \equiv \mathcal{X} \subset \mathcal{V}^+$. $\mathcal{A}$ 看情况 (可能是 next token $y_t \in \mathcal{V}$, 也可以是整条回答 $y \in \mathcal{Y}$).
    - $y_t$ 代表回答的第 $t$ 个 token, $y_i$ 代表对问题 $x$ 的第 $i$ 次 rollout, $|y|$ 或 $T$ 代表这次回答的长度.

- $\pi_\theta, \pi_T$ 代表 student 和 teacher, $\pi: \mathcal{S} \to \mathcal{A}$
    - Student 是要训练的模型, teacher 冻结. (不一定 student 比 teacher 弱)
    - $\pi_\text{ref}$ 是 old student.
    - $\pi_\theta(y|x)$ 和 $\pi_\theta(y_t|x,y_{<t})$ 代表 sequence-level 和 token-level 的概率分布.

- $r(s,a)$ 代表环境对状态动作对 $(s,a)$ 给出的 reward.

- 我们需要 maximize $J$, minimize $L$.

- 本文简化所有求期望的操作, 思考时注意是在什么样本空间上取期望!
...
Off-method 的动机就是**让 $\pi_\theta$ 模仿 $\pi_\text{ref}$ 的行为**:

$$
\boxed{
\begin{aligned}
L_{\text{KD}} :=& \mathbb{E}_{x} \left[ \text{KL} (\pi_\text{ref}(\cdot |x), \pi_\theta(\cdot |x)) \right] \\
=& \mathbb{E}_{xy_1} \left[\log \pi_\text{ref} (y_1|x) - \log \pi_\theta (y_1|x) \right] 
\end{aligned}
}
$${#eq-loss-kd}

注意上面 $y_1 \sim \pi_\text{ref}(\cdot|x)$.

而 SFT 等价于用 $\pi_\text{ref}$ 坍缩后的分布来监督, 所以 $\log \pi_\text{ref}(y_1|x)=0$, 而且不需要对 $y_1$ 进行采样了 (或者在一个 0 熵的分布上采样):

$$
\boxed{
L_\text{SFT} := \mathbb{E}_{x} \left[- \log \pi_\theta (y_1|x) \right]
}
$${#eq-loss-sft}

举个例子, 假设在 $t$ 位置 4 个 token 的概率分布如下:
...
\boxed{
\begin{aligned}
L_\text{OPD} :=& \mathbb{E}_{x} \left[\text{KL} (\pi_\theta(\cdot |x), \pi_\text{ref}(\cdot |x)) \right] \\
=& \mathbb{E}_{xy_1} \left[\log \pi_\theta (y_1|x) - \log \pi_\text{ref} (y_1|x) \right]
\end{aligned}
}
$${#eq-loss-opd}
