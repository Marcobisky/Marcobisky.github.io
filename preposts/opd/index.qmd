...
$$
-L_\text{PG} = \sum_{y} \pi_\theta(y|x) \cdot r(x,y)
$$
...
$$
\begin{aligned}
- \nabla L_\text{PG} &= \sum_{y} \nabla \pi_\theta(y|x) \cdot r(x,y) \\
&= \sum_{y} \underbrace{\pi_\theta(y|x) \nabla \log \pi_\theta(y|x)}_{\text{log trick}} \cdot r(x,y) \\
...
$$
-L_\text{PG}^\text{surr} := \log \pi_\theta(y|x) \cdot r(x,y)
$${#eq-reinforce}
...
我们把利用 @eq-reinforce 进行反向传播的算法称为 REINFORCE, 这是 PG 的最原始的形式, 后面会有很多方法对它的效果进行改良.

### REINFORCE + Baseline

(REINFORCE 有梯度在每个方向上的方差很大)

我们用:

$$
-L_\text{PG}^\text{baseline} := \log \pi_\theta(y|x) \cdot (r(x,y) - b(x))
$${#eq-reinforce-baseline}

即只添加了一个只依赖 $x$ 的函数 $b$ (**任何只依赖 $x$ 的函数都可以!**). 可以这么做的原因是后一项的梯度为 0:

$$
\begin{aligned}
\nabla \log \pi_\theta(y|x) \cdot b(x) &= b(x) \cdot \sum_y \underbrace{\pi_\theta(y|x) \nabla \log \pi_\theta(y|x)}_{\text{log trick}} \\
&= b(x) \cdot \sum_y \nabla \pi_\theta(y|x) \\
&= b(x) \cdot \nabla \sum_y \pi_\theta(y|x) \\
&= b(x) \cdot \nabla 1 \\
&= 0
\end{aligned}
$$

为了展示仅从 @eq-reinforce-baseline 读出具体是如何训练的, 假设我们用 $G$ 个样本、一个 batch size 进行训练:

$$
-\hat{L}_\text{PG}^\text{baseline} := \frac{1}{G} \sum_{i=1}^G \log \pi_\theta(y_i|x) \cdot \hat{\Psi}_i
$$
...
$$
\begin{aligned}
\log \pi_\theta(y_i|x) &= \sum_{t=1}^{T} \log \pi_\theta(y_{i,t}|x,y_{i,<t}) \\
\hat{\Psi}_i &= r(x,y_i) - \hat{b}(x) \\
\hat{b}(x) &= \frac{1}{G} \sum_{i=1}^G r(x,y_i)
\end{aligned}
$$

注意这次我们不能再以单样本来估计了, 要对同一个问题 $x$ rollout $G$ 次 (称为一个 group), 然后以组为单位 (**Group Relative**) 来归一化组内这些回答的 reward, 归一化后的数值称为 centered return (每条 rollout $y_i$ 都有一个 centered return $\Psi_i$).

But wait, 有一些 nuance 需要注意, 我们可以看到在估计中 $\hat{b}$ 是取决于 $y_i$ 的, 而不是独立的. 这样估出来的 $\nabla \hat{L}_\text{PG}^\text{baseline}$ 真的无偏吗? 其实是有偏的, 正如统计学中我们在估算一组 iid 数据 $\{x_i\}_{i=1}^n$ 的方差时, 如果使用的是样本均值, 需要用 $n-1$ 而不是 $n$ 作为分母 (Bessel's correction):
...
我们引入 REINFORCE + baseline 的无偏估计版本 RLOO: 它的唯一区别是第 $i$ 次 rollout 的 centered return $\Psi_i$ 是用组内其它 $G-1$ 次 rollout 的 reward 的平均值来计算的:
...
由于 on-policy 方法更新模型之后需要用新的模型来产生回答, 这个过程很慢 (如果很快的话 PPO 不会诞生), 我们希望能通过看旧的模型的输出来学习 (off-policy). 能否直接用 @eq-reinforce 但是用旧的模型来采样呢? (不能).
...
在 PPO 中, 如果我们要用几代之前的模型来采样, @eq-loss-grpo-grad 需要变成:

$$
- \nabla L_\text{PPO} = \frac{\pi_\theta(y|x)}{\pi_{\text{ref}}(y|x)} \nabla \log \pi_\theta(y|x) \cdot r(x,y)
$$
...
$$
\begin{aligned}
-\nabla L_\text{PPO} &= \frac{1}{\pi_{\text{ref}}(y|x)} \nabla \pi_\theta(y|x) \cdot r(x,y) \\
&= \nabla \left( \frac{\pi_\theta(y|x)}{\pi_{\text{ref}}(y|x)} \cdot r(x,y) \right)
\end{aligned}
$$
...
$$
-L_\text{PPO}^\text{surr} = \frac{\pi_\theta(y|x)}{\pi_{\text{ref}}(y|x)} \cdot r(x,y)
$$

但是当 $q$ 和 $p$ 的形状相差很大时 (比如 @fig-is-diff), 假如只采样一个样本, 按 $p$ 估计出来的大概率是一个负数, 而 $q$ 估计出来的大概率是一个正数. 只有当 $q$ 上采的样足够多以至于出现了 @fig-is-diff 左边那个绿色的样本时, 估计才能变得相对准确.

![$p$ 和 $q$ 形状差距太大是不好的.](is-diff.png){#fig-is-diff}

PPO 产生回答的模型和更新的模型不是同一个 (可能是同一个模型但是权重的更新差了几代), 因此需要使用 importance sampling 来进行校正, 并且我们希望旧模型的输出分布不要与新的差太远, 即我们让:
