...
![要区分 Training trajectory 和 Actual Path @hawley2025flowwithwhat.](fm-train-vs-actual.png){#fig-fm-train-vs-actual}
...
![不同的 dropout 设置对损失函数的影响.](ft-loss-dropout.png){#fig-ft-loss-dropout width=90%}
...
但是训练过就会发现不论将 $\alpha$ 调多大, 训练出来的 $\nabla V^*$ 都接近 0, 也就是学到的风一直为 0. @wang2020understandingmitigatinggradientpathologies 中的研究也发现, 如果用 @eq-pinn-total-loss 这种加权的形式来训练 PINN 的话, PDE Loss $L_{\text{PDE}}$ 会淹没掉 Boundary Loss $L_{\text{boundary}}$, 网络会疯狂地看 PDE 方程本身, 而忽略边界条件:

![@wang2020understandingmitigatinggradientpathologies 用神经网络拟合 Helmholtz 方程, 在 40000 步时, 4 个隐藏层反向传播梯度大小的统计. 蓝色的 $L_{\text{boundary}}$ 尖锐地集中在 0 附近.](helmholtz.png)

@liu2025vggflow 文中也写到「直接参数化 $V^*$ 不如参数化 $\nabla V^*$ 有效果」, 从而 @liu2025vggflow 选择了对 @eq-pde 两边求梯度, 得到一个比较复杂的 PDE:
...
动机很简单也很 ad hoc: 既然学不到边界条件, 那就先单独训练边界条件, 训完以后再用三个 Loss 进行联合训练:
...
![分两个阶段训练能让 PINN 学到边界条件的 Loss.](pinn-loss.png){#fig-pinn-loss width=90%}
