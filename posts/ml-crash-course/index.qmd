...
> 写这篇博客的动机: 学习神经网络的资源一般就是论文和书籍, 论文 not self-contained, 书籍 self-contained 但是会加入很多基础知识导致思维不连贯. 以下内容不会介绍任何数学 prerequisite, 只构建神经网络的完整思维框架.
...
世界上很多问题其实就是一个复杂的映射, 比如图像识别就是输入是图片, 输出是图片中的各种内容. 只不过这个函数存在于人类的大脑中, 无法写出显式的表达式. 如果这个表达式存在, 那么它肯定很复杂 (这里不能追求所谓的 “简洁与优美”). 人类想要强行把这个函数的表达式找到! 怎么找呢? 不可能遍历完世界上所有的函数对吧, 但是我们缩小一点范围, 用**某种特定形式**的函数来逼近所求. 也就是:
...
如果 $\mathcal{X}$ 是一张图片的话, 我们有 “相邻” 点这种概念, 也就是说输入进 $\hat{f}$ 的对象内部是有某些结构的, 但是 FCNN 并不知道这些结构.
