...
- **RMST (Reliable Multi-Segment Transport)**
    ...
    - **Directed Diffusion (DD) 路由算法**: RMST 默认有这种路由算法的存在. 如何让每个节点知道收到的数据发给谁? 由于 WSN 中数据几乎都是要发给 sink 的, 我们先让 sink 广播一个 Interest 消息 (@fig-dd (a)), 这样得到了从 sink 到每个节点的反向路径, 如果要反过来发的话直接将每个箭头反过来即可.
