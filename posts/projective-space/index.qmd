...
According to @eq-homogeneous-coordinates-CP1, one would need only one complex number to specify an element in $\mathbb{CP}^1$. Does it implies that $\mathbb{CP}^1$ is just the complex line[^complex-line]? No! We missed the case where $z_2 = 0$! Similar to @sec-RP1, we define the line $\begin{pmatrix} z_1 \\ 0 \\\end{pmatrix} \in \mathbb{C}^2 - \{0\}$ belongs to the equivalence class denoted by the symbol $\infty$:
$$
...
![$\mathbb{CP}^1$ as a Riemann sphere](projective-space-CP1.png){#fig-projective-space-CP1}
