...
description: "内容正确性仅限于格院 CCD 这一门课 (更新时间 Jun 16, 2025, 23:59)"
date: 2025-06-16
image: cover.png
categories:
    - Circuit
...
- 协振时 $LC$ 串联电路相当于短路, $LC$ 并联电路相当于断路. 即电路表现为**纯电阻**, 即: $$\boxed{\Im{(Z)} = 0, \quad \text{or } \Im{(Y)} = 0}$$
    ...
    - 对 $LC$ 电路, $$Z = \frac{1}{Cj\omega_0} + Lj\omega_0 = 0 \implies \boxed{\omega_0 = \sqrt{\frac{1}{CL}}}$${#eq-resonance}
    - 对 $RLC$ 电路 (@fig-RLC-resonance), 算 $Y$ 更方便: $$Y = \frac{1}{R + Lj \omega_0} + Cj \omega_0 = 0 \implies \omega_0 = \sqrt{\frac{1}{CL}-\frac{R^2}{L^2}}$$
...
- 等价定义(或推论):
    ...
    - 不管串/并联协振, $Q$ 都可由 中心频率和 $-3\text{ dB}$ 带宽相同地表示 : $$\boxed{Q = \frac{\omega_0}{\Delta \omega}}, \Delta \omega = \omega_2-\omega_1, \quad \text{when } 20\lg|H(\omega)| = -3\text{ dB}$${#eq-q-center-over-bandwidth}
    - $\omega_0$ 是 $\omega_1$ 和 $\omega_2$ 的几何平均: $$\boxed{\omega_0^2 = \omega_1 \omega_2}$$ 但是实际上很接近算数平均, 所以可以这样算 $\omega_1, \omega_2$: $$ \begin{cases} \omega_1 &= \omega_0 - \frac{\Delta \omega}{2} \\ \omega_2 &= \omega_0 - \frac{\Delta \omega}{2}\end{cases}$$
...
[^meaning]: 这有什么意义呢? 你知道电脑里那么高的时钟频率是哪来的? 单片机的定时器可以定到宇宙毁灭? (用 frequency multipler 的连级! 见 @fig-PLL-freq-mult 和 @fig-CO-modulator).
...
> AM 调制信息在幅度里面. 而且有很多种 AM 的方法[^am-method]: **DSB-SC, DSB-WC, SSB, VSB**, etc.
...
- **目标**: 让 $e(t)$ 与 $e_c(t)$ 的 **瞬时频率差** 正比于 $e_i(t)$ 的幅度: 
    ...
    - **Deviation Ratio (DR) 偏差比**: 信息信号 $e_i$ 最大的频率 $(f_i)_{\max}$ 反映到调制信号 $e$ 的最大频偏 $(\Delta f_e)_{\max}$ 上缩小了多少: $$\boxed{\text{DR} := \frac{(\Delta f_e)_{\max}}{(f_i)_{\max}}}$$
        - **Modulation Index 调制指数**: 是 DR 的特例, 即 $E_i$ 是单音信号: $$\boxed{m_f := \frac{(\Delta f_e)_{\max}}{f_i}}$$
...
- FM 调制器电路:
    ...
    - **Crystal Oscillator Modulators**: 用晶振来产生精确的载波频率. 由于 **晶振的频率可以被跟其串联的电容稍稍改变**, 所以我们可以用一个 varicap diode 作这个电容, 再用 $e_i$ 来控制这个 varicap diode.
        ...
        - 问题: 晶体的频率调节范围非常小 ($100\text{ Hz}$), 但 FM 的频偏最好要达到 $\text{kHz}$ ($75\text{ kHz}$), 所以要用 **Frequency Multiplier 倍频器** 来增大频率 (frequency deviation 也会增大).
...
- FM 解调器电路:
    ...
    - **Pulse-Averaging Discriminators**: 注意到 FM 的幅度其实没有携带信息, 只需要观察 FM 信号 **在时间轴上的交点的密集程度** 就可以判断瞬时频率了.
        ...
        ![Pulse-Averaging Discriminators: 不用多说, 看图](Pulse-Averaging.png){#fig-Pulse-Averaging width=80%}
    ...
    - **Quadrature Detectors 正交解调器**: 用 @fig-Quadrature-Detector-XOR 将频率信息转为相位差 $\Delta \varphi$ 信息 (调整 $C_2, L$ 的值使其协振频率恰为载波频率 $\omega_0$ 就可以使: 当 $e(t)$ 的频率恰为 $\omega_0$ 时 $\Delta \varphi = 90^\circ$ (所以叫 "Quadrature"); 当 $e(t)$ 有频偏 $\Delta f$ 时 $\Delta \varphi$ 会随之线性变化), 然后用一个 PD 和 LPF 的组合 (比如 @fig-Quadrature-Detector-XOR 中的 $XOR$ 和 $RC$ 电路, 增益用 @eq-pd-gain 算) 来将这个相位差信息正比地转换为 DC 电压信息.
        ...
        ![一种用 $XOR$ 作为 PD 的正交解调器, IF input 为 @fig-Pulse-Averaging 的 B 点信号](Quadrature-Detector-XOR.png){#fig-Quadrature-Detector-XOR width=80%}
...
#### Heterodyne 外差接收机 {#sec-heterodyne}

> 既然直接处理射频频率太高了, 那就先降下来呗. Heterodyne 的核心思想就是把各种射频信号 (RF) “变频”成一个**固定**的**中频** (**IF**) 信号后再去处理. 其实就在 TRF 的 Stage 1,2 之间加了一个 **IF Section**. 这样既解决了频率太高的问题, 有让后续的电路只要固定处理一个 IF 频率就行了.
...
- **AGC (Automatic Gain Control) 自动增益控制**: 输入信号的强度可能会像 Gustav Mahler 的交响曲一样忽大忽小, 造成解调失败. AGC 就是帮你缩小响度范围的.
