...
author: Marcobisky
title: CCD 通信电路速成笔记
description: "内容正确性仅限于格院 CCD 这一门课 (更新时间 Jun 16, 2025, 17:16)"
date: 2025-06-16
image: cover.png
categories:
    - Circuit
...
> EXAMPLE 是PPT或往年的经典题目, 框起来的公式要记住, *号章节了解即可, 其他的**都有可能考**
...
- 问题描述: 给定 纯电阻内阻 $R_0$ 的交流电源 $V_0$ 和 纯电阻负载 $R_L$, 在它们中间设计一个 $T$ 形网络使阻抗匹配.
    - 既然是设计匹配网络, 就不能用纯电阻了 (@fig-matching-wrong-design), 不然能量白白消耗掉了, 只能用**纯电抗 $X$ 元件** ($L$ 或 $C$, 用 $X_p, X_s$ 表示), 它们只储能, 不耗能.
...
- 解法: ★★★★★
    - **先用电阻之比算 $Q$**: @eq-q-factor-serial-parallel-conversion 的第一个公式.
