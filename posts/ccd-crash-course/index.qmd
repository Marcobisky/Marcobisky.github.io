...
> EXAMPLE 是PPT或往年的经典题目, 框起来的公式**必须记住或独立推导**!!! *号章节了解即可, 其他的**都有可能考**
...
- **Carson's Rule 卡森规则** ★★★★★
    ...
    - **若后续有 $N$ 倍频器, 则 frequency deviation 也会放大 $N$ 倍**.
...
- FM 调制器电路:
    - **Varactor Modulators**: 其实就是直接用 VCO 来当调制器, 比如用 @fig-Colpitts, 把其中一个电容换成 $e_i$ 控制的 varicap diode 就行. (PPT 上的图在 @fig-Varactor-modulator.)
        - 缺点: 当没输入 ($e_i = 0$) 的时候, 这种用 $LC$ 当反馈的 VCO 甚至不能输出精确稳定的载波频率! 所以我们会用 **晶振 (Crystal Oscillator)**:
