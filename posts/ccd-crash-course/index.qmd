...
description: "内容正确性仅限于格院 CCD 这一门课 (更新时间 Jun 17, 2025, 12:58)"
date: 2025-06-17
image: cover.png
categories:
    - Circuit
...
> 本笔记是本人电子科技大学格院大三《通信电路设计》课程的期末考试复习笔记, 遵循个人学习习惯与轨迹, 但我将尝试保留所有概念的动机和尽可能地解释原因, 不保证覆盖全部考点, 不保证严谨性和绝对正确性.
...
![用正反馈来产生自发振荡, 振荡的根源在于内部的电噪声](positive-feedback.png){#fig-osc-feedback width=70%}
...
- **High / Low Level Modulation 高/低电平调制**: 实际调制过程中 在输入和调制信号发射之间 肯定包含信号的放大 (@fig-dsb-wc 省略了), 是先调制再放大 还是 先放大再调制呢? (都可以)
    ...
    - **High Level Modulation**: 先放大再调制, 适合高功率要求[^high-level-mod], 如 radio broadcasting 广播塔台.
...
- RF 接收机的特性参数:
    ...
    - **Sensitivity 灵敏度**: 接收机能听到多小的信号.
    - **Dynamic Range (DR) 动态范围**: 接收机能处理多大范围的输入信号强度, 一般用 $\text{dB}$ 度量.
        - 比如最小最大的信号对应的电平是 $1\text{ mV}$ 和 $1\text{ V}$, 则: $\text{DR} = 20 \lg \left(\frac{1}{0.001}\right) = 60\text{ dB}.$
...
![Heterodyne 比 TRF 多了一个中频率转换, 包含一个 Mixer 和一个 BPF, LNA 只是用来放大的](Heterodyne.png){#fig-Heterodyne width=80%}
...
- **Low Noise Amplifier LNA 低噪声放大器**: 一个以 $\omega_{\text{RF}}$ 为中心频率的放大器. 由于 RF 信号本身很弱, 后续电路听不到 (而 RF stage 只有选择作用而没有放大作用), 所以加一个放大器用来放大微弱的 RF 信号.
    - 叫做 “低噪声” 是因为一般的放大器在放大信号的同时会引入很多的**电噪声** (系统本身的噪声, 不是外界的!), 而 LNA 本身的电噪声很小.

- **Noise factor ($F$) 电噪声因子**:
    ...
    - 定义: 一个放大器输入信号的信噪比 (SNR) 与输出信号的 SNR 的比值: $$\boxed{F := \frac{\text{SNR}_{\text{in}}}{\text{SNR}_{\text{out}}} = \frac{S_i / N_i}{S_i G / ((N_i + N_a) G)} = 1+\frac{N_a}{N_i}}$$
        ...
        - **Noise Figure ($\text{NF}$)**: 仅仅是化成 $\text{dB}$, 注意 $F$ 本来就是功率比了, 所以用 $10$ 当系数: $$\boxed{\text{NF} := 10 \lg F}$$
        - 由于所有放大器都可能有电噪声 ($N_a \ge 0$), 所以 $F \ge 1, \text{NF} \ge 0$.
