...
### Matching Network Design 匹配网络设计
...
![用带通滤波器滤掉可能存在的镜像频率, 定量分析见 @sec-rlc-rejection-filter, $\omega_{\text{IF}}$ 见 @sec-heterodyne](image-rejection.png){#fig-image-rejection width=80%}
...
- 等价定义(或推论):
    ...
    - 串联协振 (记忆: 串联电流主导, $R$ 在分母, $R$ 越大越耗能, $Q$ 越小): $$ \boxed{Q = \frac{\omega_0 L}{R} = \frac{1}{\omega_0 C R}}$${#eq-serial-Q} 并联协振 (记忆: 并联电压主导, $R$ 在分子, $R$ 越大泻能越慢, $Q$ 越大): $$ \boxed{Q = \frac{R}{\omega_0 L} = \omega_0 C R}$${#eq-parallel-Q}
    ...
    - $\omega_0$ 是 $\omega_1$ 和 $\omega_2$ 的几何平均: $$\boxed{\omega_0^2 = \omega_1 \cdot \omega_2}$$ 但是实际上很接近算数平均, 所以可以这样算 $\omega_1, \omega_2$: $$ \begin{cases} \omega_1 &= \omega_0 - \frac{\Delta \omega}{2} \\ \omega_2 &= \omega_0 - \frac{\Delta \omega}{2}\end{cases}$$
        ...
        ![$\omega_0$ 是 $\omega_1$ 和 $\omega_2$ 的几何平均](RLC-BPF.png){width=60%}
