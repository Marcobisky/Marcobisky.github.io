---
author: Marcobisky
title: "PDE: Derive Wave and Heat Equations from Physics"
...
The force acting on the $n$-th ball is the sum of the forces from the two adjacent balls:
$$
\begin{aligned}
F_n &= F_L + F_R \\
...
According to Newton's second law[^ddot] $F_n = m \ddot \xi_n$, we have:
$$
...
Now @eq-longitude-discrete should be it! But in reality, we do not know $m$ and $k$ since they are microscopic parameters. We now define so new parameters to connect the quantities that we can measure and the microscopic parameters:
...
You may think the 2D analog is something like @fig-d2-longitudinal-wave. However, this kind of wave is **vector-valued** just like electromagnetic wave, where the "amplitude" $\mathbf{\xi}: \mathbb{R}^n \times \mathbb{R} \to \mathbb{R}^n$ is a time-dependent vector field. We will discuss vector-valued wave equation in the future[^curiosity]. In fact, the transverse wave (shown in @fig-d2-transverse-wave) is quite accurate for 2D scalar-valued waves.
