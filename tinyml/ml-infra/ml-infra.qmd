...
- **4D tensor**: 特别是在图像处理中, 4D 张量非常常见, 现在单独详细研究一下它:
    ...
    - **描述顺序和内存中的布局**[^data-layout]: (注意无论多少维度的张量, 在内存中显然都是以 (也只能以) 一维数组的形式连续存储).
        - <u>HWC</u>**N**: Batch 在最后.
...
- 下面代码 `Softmax(dim=1)` 表示在 `t` 的第 2 个维度「切片」的整体做 softmax, 比如:
    ...
    t = nn.Softmax(dim=1)(t)
    print(t)    # tensor([[0.2500, 0.2500, 0.2500, 0.2500],
