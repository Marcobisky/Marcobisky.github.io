...
> 一般有参数的层放在 `__init__` 里, 没有参数的层放在 `forward` 里. 
...
- `torch` 中自带的 `MultiheadAttention` 要求输入必须为 `NLE` 格式, 其中 `N` 是 batch (可以同时处理多个句子!), `L` 是句子长度, `E` 是 embedding 维度. 下面的例子由 @fig-mha-compute 诠释:

    ```python
    t = torch.randn(1, 4, 6)
    attn = nn.MultiheadAttention(embed_dim=6, num_heads=3)
    output, _ = attn(t, t, t)  # Self-attention
    print(output)
    ```
...
- 关于 Embedding 维度和 Value (或 Query, Key) 维度的关系:
    - 默认 **Embedding 维度 = Value 维度!** (Single-head attention 下).
        ...
        ![3b1b 在解释 Single-head attention 的时候用了 12288 的 Embedding dimension 和 128 的 Query dimension, 这是错误的. Query 的维度这个数据是针对 GPT-3 的 96 head 的配置 ($96*128=12288$).](attention-dim-3b1b.png){#fig-attention-dim-3b1b width=70%}
...
- 注意 `MultiheadAttention` 里面还有一个不改变维度的全连接层 $W_O$! 这个并不是 3b1b 视频中的 MLP, 真正的 attention block 还包括 `LayerNorm`, `Dropout`, Residual Connection. 下面的代码对应 @fig-pre-post (b), **非常重要!!!**
