...
- **Network**: 输入图片和时间, 输出速度, $(\mathbf{x}, t) \mapsto \mathbf{v}$.
    - 相当于找一个时变向量场 (non-stationary vector field).
    - 一般规定 $t \in [0,1]$.

- **一条训练数据的构建**: 在正态分布和 $p_{\text{data}}$ 中分别随机选一个点构成 $(\mathbf{x}_0, \mathbf{x}_1)$ pair, 它们连线上的任意一点 $\mathbf{x}_t$ 都可以构建一条训练数据 $(\mathbf{x}_t, t, \mathbf{v}_t)$.
    - $\mathbf{v}_t$ 的方向就是简单粗暴地从 $\mathbf{x}_0$ 指向 $\mathbf{x}_1$.

        ![用直线移动训练模型 @hawley2025flowwithwhat.](fm-train.png){#fig-fm-train width=70%}
    
    - 训练后的向量场不会使得轨迹是直线而是曲线!

        ![要区分 Training trajectory 和 Actual Path @hawley2025flowwithwhat.](fm-train-vs-actual.png){#fig-fm-train-vs-actual width=80%}
