...
- `orR`: reduction OR, 对所有位进行 OR 操作. 比如 GPR 读寄存器时, 只要地址不是全 0，就读出寄存器的值:
...
{{< include AddSubSimple.scala >}}
```

```{.scala filename="AddSubArea.scala" #lst-AddSubArea}
{{< include AddSubArea.scala >}}
```
...
{{< include AddSubSimple.v >}}
```

```{.verilog filename="AddSubArea.v"}
{{< include AddSubArea.v >}}
```
