...
- C++ 里只有一维数组的元素是连续存储的!
- C++ 里没有真正的二维数组, 只有一维数组的指针 (array of pointers).
...
- 在 Heap 上动态分配:
    ...
    int main (void) {
        ...
        int** p = new int*[4]; // purple p on stack
...
- `std::unique_ptr<T>`: 不能被复制:
    ...
    ```cpp
    std::unique_ptr<int> p1 = std::make_unique<int>(42); // 在堆上分配一个 int, 初始化为 42, 并将其地址赋给智能指针 p1
...
::: {layout-ncol=2}
```{.cpp filename="copy_sharedptr.cc" #lst-copysharedptrcc}
{{< include src/cpp/copy_sharedptr.cc >}}
```

```{.cpp filename="copy_wildptr.cc" #lst-copywildptrcc}
{{< include src/cpp/copy_wildptr.cc >}}
```
:::
...
- **Constructor 构造函数**: 相当于 `Python` 里的 `__init__` 方法. 每次创建对象时会被调用.
    - 可以有多个 Constructor (根据初始化时所带参数数量和类型自动调用, 也是**多态**的一种体现). 名字必须直接用类名: `ClassName()`.
        - 也支持 **Default Arguments 默认参数**: `ClassName(int w = 1)` 意思是如果创建对象时没传参数 `w` 就等于 `1`.
...
- 下面代码表示 `class A` 继承 `class B` (记作 "A is-a B").
    - `public` 表示**公开继承**, 即 `B` 的 `public` 和 `protected` 成员在 `A` 里依然是 `public` 和 `protected`[^protected].
...
- 我们想用同一个函数 `print_res()` 来打印 `A` 类和 `B` 类 (derived from `A`) **各自**的 `forward()` 结果, 我们在 C++ 和 Python 里实现它们. 发现虽然两种语言 `print_res()` 定义时规定参数类型都是 `A*`, Python 就很聪明, 成功调用了各自的 `forward()` 方法; 而 C++ 却都用的是 `A` 类的 `forward()` 方法 (这不是我们所期望的).
...
- 实现: 在父类的方法后面前加上 `= 0`, 称为一个 **Pure Virtual Function 纯虚函数**.
    ...
    - @lst-virtualpurecc 还实现了 **Wrappers 包装器**, 用于给已有的类添加功能 (如同 Python 装饰器).
...
- 全局作用域运算符:
    - 所以自己的 namespace 最好避开有可能称为官方 namespace 的名字, 比如 `namespace cuda { ... }`.

```{.cpp filename="namespace-global.cpp" #lst-namespaceglobalcpp}
{{< include src/cpp/namespace-global.cpp >}}
```
