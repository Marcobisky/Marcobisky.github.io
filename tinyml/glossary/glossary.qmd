...
- **CSR (Compressed Sparse Row)**: 用 index + data 进行**连续**存储的格式 (E.g., sparse matrix, [pin-list 表示法](../eda-3d-placement/eda-3d-placement.qmd#sec-pl), etc.)
...
- **STA (Static Timing Analysis)**: 静态时序分析 @liao_2023_dreamplace. 静态指通过线长等物理结构直接分析电路的 delay (而不需要通过仿真).
    - **Slack (时序)裕量**: 信号在导线上的传输是需要时间的, slack = 信号要求到达时间 - 信号实际到达时间. $\text{slack} > 0$ 安全, $\text{slack} < 0$ 违规 @liao_2023_dreamplace.
    ...
    - **TNS (Total Negative Slack) $s_{\text{tns}}$**: 所有违规 slack 相加 @liao_2023_dreamplace (如果没有违规, 则 $s_{\text{tns}}$ 不存在).
        - 一般 WNS 和 TNS 都只关心 Endpoint pins (如 flip-flop inputs, outputs ports @liao_2023_dreamplace) 中的违规情况.
