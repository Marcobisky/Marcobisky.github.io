...
[^gd]: *Abuse of Notation* 一般说的 GD/SGD 指 Mini-batch 版本, 实际训练中不用 BGD (计算量太大) 和纯 SGD (不稳定)!

- **Momentum-based 动量优化器**: 引入历史项的加权平均, 相当于给参数中的点赋予质量和惯性 (而不是没有质量), 可加速和平滑收敛、避免陷入局部最优. 也有一些基于次改良的版本:
