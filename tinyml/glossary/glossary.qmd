...
- **FE/BE (Front/Back End)**: 前端/后端. 要灵活理解它的意义 (TODO).
...
- **JTAG 接口**: 一种用于调试和编程的标准接口 (TDI, TDO, TCK, TMS)
    - JTAG 组件存在于一颗芯片 (比如 CPU) 内部, 对于芯片来说是透明的 (不知道 JTAG 的存在).
    - JTAG 可用于读出所有引脚的值、向芯片内部的 flash 写入数据、甚至阻止 CPU 控制引脚的电平输出转而由 JTAG 来控制输出. 参考 [这篇 Medium 博客](https://medium.com/@aliaksandr.kavalchuk/diving-into-jtag-protocol-part-1-overview-fbdc428d3a16).
...
- **Cinebench**: 一般是测试 CPU 的多线程、多核心能力, 通过渲染一个复杂的 3D 场景 来测量硬件执行浮点运算和并行任务的能力.
...
- [**iverilog (Icarus Verilog)**](https://github.com/steveicarus/iverilog): 可直接输出波形的真正的仿真器 (不输出 C 代码). 由于是解释型的, 速度比 `verilator` 慢很多. 用法:
    ...
    ```bash
    # 生成可执行的临时仿真文件 blink.vvp, 顶层模块是 blink.
    iverilog -o blink.vvp -s blink blink.v blink_tb.v
    # 生成 vcd 文件 (可配合 GTKWave 查看波形)
    vvp blink.vvp
    ```
...
- **LEF (Library Exchange Format)**: 描述 cell 的物理信息 (尺寸、形状、pin 位置、所在金属层, 是否镜像 (SYMMETRY), etc.)

- **DEF (Design Exchange Format)**: 描述整个芯片的 placement 和 routing.

- **GDSII (Graphic Data System II)**: 版图文件 (最终交给晶圆厂的文件).
...
- **Power delivery network (PDN)**: 电源分配网络.
    - **Power rail 电源线**: 给晶体管上电的金属线.
    - **Backside power delivery (BSPD)**: 通过芯片背面供电 (而不是跟信号线在同一侧), Intel 独占的技术 (截止 2025)
        - **Power via**: 通过 TSV 伸到晶体管侧面供电 (比如 Intel Panther Lake @11075006).
        - **Power direct**: 直接插入晶体管供电.

    ![BSPD 示意图 @Hossen2020PowerDN.](bspd.png){#fig-bspd width=90%}

- **IR Drop**: 芯片供电网络 供电引脚 和 内部晶体管 上的电压不是完全一样的 (因为 PDN 的 Current 和 Resistance), 可将最坏情况定义为 IR Drop. 如果 IR Drop 过大, 芯片逻辑会出错或无法工作.

- **Congestion 拥塞**: 在芯片的某个局部区域 routing 时, 发现空间不够或很拥挤 (需要连接的金属线数量, 超过了该区域实际能容纳的金属线数量).
...
- **pitch 间距**: 两个相邻的重复元件中心之间的距离. 很多时候指金属层走线的最小间距 @ysyxvideos_2025_physical. HBT 的优点就是 pitch 比较小 (fine), 集成度高.
...
- **CUDA Core/SP, SM, GPC; thread, warp, block, grid** 等: 见 [GPU 原理速成](../cc-gpu/cc-gpu.qmd).
...
- **CPython**: 用 C 写的 Python 解释器.
    - Python 的底层原理体现了「**程序即数据**」的思想, Python 代码 (仅仅是一段文本) 首先被解析为 **AST**, 然后被处理为 **Bytecode** (包含 `opcode`, `oprand` 等). CPython 逐条读取 Bytecode (相当于访问数据), 然后执行对应的操作.
        ...
        ```
    - Question: 既然 Python 的底层是 C, 为什么在 RISCV 平台上移植 Python 有困难?
        - 首先运行纯 Python 没有困难.
        - 有困难的是 Python 的第三方库 (如 NumPy, SciPy, Pytorch), 因为它们的底层实现是 C++/CUDA 编译出来的二进制, RISCV 根本不认识这些指令.
...
- **Primitive 原语**: 一段不能被中断打断的程序 (属于原子操作). 
...
- **OEM (Original Equipment Manufacturer)**: 原始设备制造商 (代工厂或贴牌制造商).
