...
- **FIRRTL (Flexible Intermediate Representation for RTL, `.fir`)**: 一种用于硬件设计的中间表示. 可由 Chisel 生成, 然后再转换为 Verilog.
...
- [**Cocotb**](https://www.cocotb.org): 与 `verilator` 和 `icarus` 不同, `Cocotb` 在它们之上, 用于方便地写测试逻辑而不是模拟硬件.
