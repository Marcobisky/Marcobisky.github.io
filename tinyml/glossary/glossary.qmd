...
- **Wafer 晶圆, Die, Chiplet Arch**
    ...
    ![Die & Wafer 关系, Chiplet 思想 (Use smaller dies) 与其如何提高良率、降低成本 @filho_2024_what](chiplet.png){#fig-chiplet}
...
- **Row**: 由于供电的原因, cell/macro 不能随便放置, 而要在某个方向对齐, 使得有“一层一层”的概念, 每一层称为一个 row. Row height 一般等于 std cell 的高度.
    - **Legalization**: Global (coarse) placement 的结果是随便放置的, 将 cell/macro 吸附到最近的 row 上并推开重叠 (用 Tetris 或 Abacus 算法) 的过程称为 Legalization (见 @fig-legalization).

:::{.column-margin}
![Legalization 把 cell/macro 吸附到最近的 row 上并推开重叠 @phdthesisAlgorithms2019](legalization.png){#fig-legalization}
:::

- **Bin**: 仅仅是为了方便 density 的**计算** (类似有限元方法), 整个 die 被划分为若干个小的矩形区域 (厚度跟 die 的厚度一样), 每个小区域称为一个 bin @zhao2024analyticalheterogeneousdietodie3d (见 @fig-bindemo). 所有的 bin 组成的集合记作 $B$.

:::{.column-margin}
![此图中 bin 的高度是 row height 的 $4$ 倍 @phdthesisAlgorithms2019](bindemo.png){#fig-bindemo}
:::


### Timing Analysis

- **STA (Static Timing Analysis)**: 静态时序分析 @liao_2023_dreamplace. 信号在导线上的传输是需要时间的, 静态指通过线长等物理结构直接分析电路的 delay (而不需要通过仿真).
...
- **Timing arc**: 若 pin $i$ 的信号变化会影响 pin $j$ 的信号变化, 则称 pin $i$ 到 pin $j$ 之间存在一个 timing arc (注意有方向).
    ...
    - **Net arc**: 金属层走线或 via 上的 timing arc.
    - **Arc delay**: 若 $(p_i, p_j)$ 间存在 timing arc, 则 arc delay 表示从 pin $p_i$ 信号变化到 pin $p_j$ 信号变化所需的时间.


- **Slack (时序)裕量**: Timing arc 不可避免, 但 arc delay 必须满足要求即可. slack = 信号要求到达时间 - 信号实际到达时间. $\text{slack} > 0$ 安全, $\text{slack} < 0$ 违规 @liao_2023_dreamplace.
...
- **Momentum-based 动量优化器**: 引入历史项的加权平均 (等价于指数加权), 相当于给参数中的点赋予质量和惯性 (而不是没有质量), 不易受噪声影响, 可**加速**和**平滑**收敛、避免**陷入局部最优**. 也有一些基于次改良的版本:
