...
- **Post-training/Fine-tuning 后训练/微调**: 预训练模型只会续写不会回答问题, 在预训练模型的基础上, 用相对少量标注数据对模型进行训练, 以适应特定任务 (比如电影评论->情感标签, 提问->回答).
    ...
    - **Instruction tuning/Supervised fine-tuning 指令微调/有监督微调 (SFT)**: 用**指令-回答对数据集**微调 @tcheyan_2025_decentralized, 如 "翻译成中文: How are you?" -> "你好吗?", 让模型会回答而只是文本补全. 但是这样的数据集太小了, 因为让人写回答太慢了, 但是**让人判断哪种回答更好很简单**, 于是有了 RLHF:
    - **Alignment tuning/Reinforcement Learning with Human Feedback (RLHF) 对齐微调/基于人类反馈的强化学习**: RLHF 让模型给出多个回答, 然后让**人类**打分 (Reward), 用强化学习 (如 PPO) 进行微调来最大化这个 Reward @tcheyan_2025_decentralized.
        - **Reward Model**: 甚至不用人类来打分, 为了创造更大的 “数据集”, 可以用人类打过分的数据集训练一个会打分的模型 (Reward Model), 然后这个模型就能给新的指令-回答对打分了 (而不用人类了).
