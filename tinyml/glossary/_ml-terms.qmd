...
- **Pretraining 预训练**: 用维基百科、书籍等未标注的超大规模数据集对模型进行训练 (自监督的), 这将会占用大量计算资源和时间 (比如几个月). 方法包括:
    - **Masked Language Modeling (MLM)**: 随机挖空, 然后预测挖空的词语.
    - **Next Sentence Prediction (NSP)**: 以句子为单位的 MLM.

- **Post-training/Fine-tuning 后训练/微调**: 预训练模型只会续写不会回答问题, 在预训练模型的基础上, 用相对少量标注数据对模型进行训练, 以适应特定任务 (比如电影评论->情感标签, 提问->回答).
    - **Full fine-tuning 全参数微调**: 更新模型的所有参数, 且每个参数的变化是**独立自由**的. 性能好, 但成本极高 (700 亿参数的 LLM 需要 1TB 显存).
    - **Parameter-efficient fine-tuning (PEFT) 参数高效微调**: 主流, 可以选择更新模型某些模块的参数 (从代码来看, 可以在 `target_modules` 上设置 `"q_proj", "k_proj", "v_proj"`), 而且需要更新的参数的自由度会小很多 (比如 **LoRA** 由 rank 超参数决定).
    - **Instruction tuning/Supervised fine-tuning 指令微调/有监督微调 (SFT)**: 用**指令-回答对数据集**微调 @tcheyan_2025_decentralized, 如 "翻译成中文: How are you?" -> "你好吗?", 让模型会回答而不只是文本补全. 但是这样的数据集太小了, 因为让人写回答太慢了, 但是**让人判断哪种回答更好很简单**, 于是有了 RLHF:
    - **Alignment tuning/Reinforcement Learning with Human Feedback (RLHF) 对齐微调/基于人类反馈的强化学习**: RLHF 让模型给出多个回答, 然后让**人类**打分 (Reward), 用强化学习 (如 PPO) 进行微调来最大化这个 Reward @tcheyan_2025_decentralized.
        - **Reward Model (RM)**: 甚至不用人类来打分, 为了创造更大的 “数据集”, 可以用人类打过分的数据集训练一个会打分的模型 (Reward Model), 这个模型输入 (Prompt, Response), 输出一个 score (相当于一个人类打分者).
            - **Comparison Data**: 不同的人会给不同的 (Prompt, Response) pair 打不同的分, 但是人类针对哪个回答**更好**一般是一致的 (OpenAI's labelers 的 inter-labeler agreement 为 73%), 这样数据集就变成了 (prompt, winning_response, losing_response).
    - **DPO (Direct Preference Optimization) 直接偏好优化**

- **Alignment 对齐**: 不仅让模型会回答问题/生成满足 prompt 的图片, 还要让它的回答更好.
    - 但是和 Post-training 有交叉, 不是完全功能独立的.
...
- **自动驾驶相关**:
    - 研究方式:
        - **闭环**: 需要用到仿真器 (如 CARLA) 来模拟自动驾驶的环境, 模型的输出会反过来影响输入 (所以叫闭环).
        - **开环**: 不需要仿真器, 用录制好的真实驾驶的视频来训练模型, 模型是在被动地学习, 输入不会因为模型的输出而改变 (所以叫开环).
            - 开环中用来评估模型好坏的指标: **L2 distance** (预测轨迹与真实轨迹的 L2 距离), **Collision rate** (预测轨迹与其它物体发生碰撞的频率).
    - 相关模型:
        - **BEV Net (Birds-Eye-View)**: 一系列网络模型 (而不是一个).

- **ViT (Vision Transformer)**: 一种基于 Transformer 架构的计算机视觉模型.

- **AIGC (AI-Generated Content)**: 生成式 AI.

- **PINN (Physics-Informed Neural Networks)**: 比如用神经网络来预测 Navier-Stokes 方程的解, 神经网络的输入为 $(x,y,z,t)$, 输出为 $(u,v,w,p)$ (三个方向的速度和压强), 然后基于这些数据可以得到一些偏导数的信息, loss 函数由两个方面构成: 少量的数据点、以及 Navier-Stokes 方程本身的约束条件.

- **End-to-end model 端到端模型**: 不拆解问题而是直接用一个步骤来解决. 
    - 比如同声传译, 标准化流程需要设计 3 个网络 (ZNspeech -> Chinese -> English -> ENspeech); 而端到端模型直接是 ZNspeech -> ENspeech.
    - 再比如自动驾驶, 端到端直接就是图像/传感器输入 -> 油门/方向盘角度输出, 用一个网络解决, 没有明确的分工网络.

- **Prompt Engineering 提示词工程**: 通过**设计一个模版**告诉模型额外的信息.
    - 为什么要设计模版呢? 比如训练 CLIP 模型需要图片-文本对, 如果发现某个文本模版少了一些信息, 就需要补上, 怎么补上呢? 比如是一张遥控器 (remote) 的图片, 直接把 “remote” 加到原始文本的后面? 肯定不行, 这样句子都不完整, 而且模型可能会觉得 “remote” 是 “遥远的” 意思, 这时候就需要用 “A photo of a remote” 这样的**模版**来补全这个文本, 这就是 prompt engineering.

- **Embedding space and Latent space 嵌入空间和潜空间**: 一样的意思, 表示将 token 或高维的数据映射到一个新的低维空间 (这个过程叫 embedding / encoding / dimensionality reduction / feature extraction / manifold learning / feature representation, etc. 真会起名字, 无语了), 空间中的元素叫做 embedding vector.
    - 如果真要说有区别的话, **latent space 更 intrinsic 一点**, 毕竟 embedding 感觉可以潜入欧式空间一点.

- **World Models 世界模型**
    - **JEPA (Joint-Embedding Predictive Architecture)**: Yann LeCun 提出的一种世界模型的思想/模型架构. 大概意思是不要期待模型逐帧/逐像素生成, 而是在一个抽象的语义空间中预测和生成 (有点 Latent Diffusion 的感觉?)

### RL 相关

- RL 中不同算法的一个很重要的区别是 reward 的来源, 根据它, RL 可分为下面两个大类:
    - **RLHF (Reinforcement Learning with Human Feedback)**: Reward 来源于 hman preference reward model.

    - **RLVR (Reinforcement Learning with Verifiable Rewards)**: Reward 来源于 rule-based or deterministic verifiers.
...
- **Schrödinger Bridge (SB) Problem**: 在所有把分布 $p_0$ 变成 $p_1$ 的随机过程中, 找到一个最接近 Brownian motion 的过程. 
    - 对于所有的随机的东西, 都有一种方法将它变成确定的、能把握的东西, 这就是**取平均** (与这个思想有关的概念包括不限于: Probability Flow, Distribution, etc.)
    - 随机过程有一种理解方法是剔除时间这个变量, 这样随机过程就退化为了随机变量, 可测空间是所有路径的空间 (Sample path space), **路径分布** 就是在路径空间上的测度. 注意这里路径空间是很大的, 而且是连续的, 所以「一条」路径没有概率, 只有概率密度.
    - SB 问题可以 (不严格地) 表述为: 已知 Brownian motion 的路径分布 $R$, 求另一个路径分布 $P$, s.t. $P$ 的边缘分布分别是 $p_0$ 和 $p_1$, 同时 $\text{KL}(P \| R)$ 最小. 
    - 当 $p_0$ 为 Gaussian distribution, $p_1$ 为图片的分布时, SB 问题的解就是一个 diffusion model.

- **Wasserstein Distance (WD)**: 也叫 Earth Mover's Distance, 衡量了两个分布之间的距离, 直观理解是把一个分布的「土」搬到另一个分布需要的最小「工作量」.
    - 看 [this video](https://www.youtube.com/watch?v=CDiol4LG2Ao) 即可理解.
    - WD 可以看成对 $\lVert \cdot \rVert_\infty$ 的优化.
    - WD 和 $\lVert \cdot \rVert_\infty$ 都是 distance. 但 KL divergence 不是 distance (不满足对称性和三角不等式).
...
- **Model States**: 训练时模型本身相关的、必须存储的参数. 包括 Parameters, Gradients, Optimizer States (比如 Adam 中的 momentum 和 variance).

- **Residual States**: 训练过程中非模型必须的数据: Activation, Temporary buffers, Unusable fragmented memory (碎片化存储空间).

- **数据类型**:
    - **FP32**: 32 位浮点数, 1 位符号位, 8 位指数位, 23 位尾数位 (**E8M23**), 精度高, 计算速度慢.
        - **FP16**: E5M10.
    - **BF16**: Brain Floating Point 16, 1 位符号位, 8 位指数位, 7 位尾数位 (**E8M7**), 精度低, 计算速度快.
    - 对于小数的表示, 16/32/64 位也叫: **半精度/单精度/双精度**.

- **混合精度**: 一般指 FP32 + FP16/BF16.
...
- **Spase DNN**: 稀疏神经网络, 指网络中有大量的权重为零 (即不参与计算) 的神经网络.
    ...
    - **Pruning**: 剪枝, 将神经网络中不重要的权重 (如接近零的权重) 设置为零.
        - **Unstructured Pruning**: 非结构化剪枝, 移除个别权重, 硬件控制复杂度大, 很少用.
        - **Structured Pruning**: 结构化剪枝, 移除整个通道/滤波器/神经元等 @sabih2025hardwaresoftwarecodesignriscvextensions, 更适合硬件加速. (比如 @fig-sparsity (a) 就可通过 pruning 移除输入的绿色神经元和输出的红色神经元).
            - **L1-norm pruning**: 对每个 filter 的权重求 L1 范数, 然后直接丢弃 L1 范数最小的若干个 filter (对应的输出通道也丢弃).
        - **Semi-structured Pruning**: 半结构化剪枝, 介于上述两者之间.

- **CIM (Compute In Memory)**: 存内计算.
    - 相关概念: **PIM/PNM (Processing In/Near Memory)**.
...
- **TBE (Tensor Boost Engine)**: 华为自研的、专门用于在昇腾 AI 处理器 (NPU) 上编写和运行高性能计算算子 (Operator) 的核心组件.


- **BLAS (Basic Linear Algebra Subproblems)**: 计算机的几乎所有的数值计算 (QR 分解, 神经网络前向传播, etc.) 最终都会退化成以下三种形式 (当然前两种是第三种的特殊情况). BLAS 就是定义下面三个 level 的规范. (*cuBLAS* 是 NVIDIA 的 BLAS 实现, Apple 的 BLAS 实现是 *Accelerate*).
    - **Level 1**: 数据访问量 $\mathcal{O}(N)$ 的系列操作, 包括:
    
    $$
    \begin{aligned}
    \mathbf{v} \mathrel{{+}{=}}& \alpha \mathbf{w} \\
    a =& \mathbf{v} \cdot \mathbf{w} \\
    a =& \lVert \mathbf{v} \rVert
    \end{aligned}
    $$
    - **Level 2**: 数据访问量 $\mathcal{O}(N^2)$ 的系列操作, 比如矩阵-向量乘法:

    $$
    \mathbf{v} = \alpha \mathbf{A} \mathbf{w} + \beta \mathbf{v}
    $$
    - **Level 3**: 数据访问量 $\mathcal{O}(N^3)$ 的系列操作, 比如 **GEMM (GEneral Matrix-Matrix Multiplication, 通用矩阵乘法)**:

    $$
    \mathbf{C} = \alpha \mathbf{A} \mathbf{B} + \beta \mathbf{C}
    $$

- **CUDA Toolkit**: NV 发布的 CUDA 工具包, 包含了 CUDA 编译器 (NVCC)，CUDA线性代数库 (cuBLAS) 等组件. 按照 [官方提示](https://developer.nvidia.com/cuda-downloads) 安装.
...
- **Gradient Descent (GD) 梯度下降[^gd]**: 所有优化方法都从这个基础方法改进而来.
    ...
    - **Batch**: 每次用多少样本进行一次参数更新.
        - Batch 其实就是数据集的一种**分割** (如果数据集小的话就不分割了, 直接用整个数据集). 分割的意思就是每个 batch 中的东西不会重复:
            ```python
            data = [1,2,3,4,5]
        ...
        - **注意区分 batch 和 epoch**: 一个 epoch 是指整个训练集的每一条都用过了, 10000 个 epoch 就是每条相同的数据都用了 10000 次. 一个 epoch 内部可能会分很多个 batch (也可以只有一个 batch, 即整个数据集都喂进去, 即 BGD).

[^gd]: *Abuse of Terms:* 现在说的 GD/SGD 就是指 Mini-batch 版本! 实际训练中不用 BGD (计算量太大) 和纯 SGD (不稳定)!

- **Optimizer 优化器**: 更新模型参数的算法.
    - **Momentum-based 动量优化器**: 引入历史项的加权平均 (等价于指数加权), 相当于给参数中的点赋予质量和惯性 (而不是没有质量), 不易受噪声影响, 可**加速**和**平滑**收敛、避免**陷入局部最优**[^momentum]. 也有一些基于此改良的版本:
    ...
    - **Adam 优化器**: Momentum 和 Adagrad 的结合 (用过去梯度决定方向, 用梯度方差决定 lr).
...
- **Tensorflow, JAX, PyTorch, MindSpore (华为)**: 机器学习框架 @fig-mlsys. 其实就是一些 Python 库.
...
- **vLLM**: 也是一个 Python 库, 专门用来加速大模型的 inference.
...
- **Triton related**: Pass (Operation Pass, Function Pass, Module Pass, Loop Pass, Analysis Pass), Pass 管理器, `@triton.jit` 机制, MLIR 通用优化 (CSE, DCE, Inlining, etc.), Triton IR, Triton IR to TritonGPU, TritonGPU 优化 (Coalesce, Matmul acceleration, Pipeline, Prefetch, Remove layout, etc.), CTA, Linalg, Layout (CTA Layout, Shared Layout, Blocked Layout, NvidiaMma Layout, DotOperand Layout, Slice Layout), MLIR

### LLM

- **Chat Template**

    ![](chat-template.png){#fig-chat-template width=80%}

- **Logits**: Logits 经过 softmax 得到 probability.
...
- 模型压缩
    ...
    - **NK (Knowledge Distillation 知识蒸馏)**: 通过训练一个小模型 (student, Net-S) 来模仿一个大模型 (teacher, Net-T) 的行为, 从而达到压缩模型的目的. 


### Benchmarks in ML

- **ROC (Receiver-operating characteristic curve)**: 二分类最后都会有一个 hard threshold, 改变这个 threshold 就会得到不同的 FPR/TPR (见 @fig-pre-recall), 以 FPR 为横轴, TPR 为纵轴画出的曲线就是 ROC curve.

- **AUC (Area Under Curve)**: 二分类模型的性能评估指标, ROC curve 下的面积, 越大越好.
    - AUC = 0.5 代表随机猜测, AUC = 1 代表完美分类器.

- **F-score**: 二分类 (正类、负类) 模型的性能评估指标.
    - **TP (True Positive)**: 正类被正确分类为正类.
    - **FP (False Positive)**: 负类被错误分类为正类. (也叫 Type I error)
    - **FN (False Negative)**: 正类被错误分类为负类. (也叫 Type II error)
    - **Recall 召回率**: $TP/TP+FP$
...
::: {.column-margin}
![应用场景](F-score.png){#fig-f-score}

![Precision (FPR, False Positive Rate) 和 Recall (TPR, True Positive Rate)](pre-recall.png){#fig-pre-recall}
:::
