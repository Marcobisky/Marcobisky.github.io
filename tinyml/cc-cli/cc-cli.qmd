...
```bash
nohup python main.py # no hang up, 让程序在后台运行, 即使关闭终端也不会停止
ps aux | grep python # process status, 查看所有正在运行的 python 程序
kill <PID> # 终止指定进程, PID 是进程 ID
```

- `a`, `u`, `x` 是参数, 代表: All users, User-oriented format, include eXtra processes without a controlling terminal.
